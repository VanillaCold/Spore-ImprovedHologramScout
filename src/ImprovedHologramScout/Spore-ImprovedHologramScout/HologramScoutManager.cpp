...
bool HologramScoutManager::WriteToXML(Simulator::XmlSerializer* stream)
{
	return false;//stream->AttributesToXml(ATTRIBUTES, this, this->GetName());
}
...
HologramScoutManager* HologramScoutManager::sInstance;

HologramScoutManager* HologramScoutManager::Get()
{
	return sInstance;
}
...
void HologramScoutManager::Update(int deltaTime, int deltaGameTime) 
{
	...
	else
	{
		...
		if (HologramUIManager::Get()->mpLayout)
		{
			HologramUIManager::Get()->CloseUI();
			HologramUIManager::Get()->mpLayout = nullptr;
		}
...
void HologramScoutManager::GetPlayerInput()
{
	...
	if (GameInputManager.IsTriggered(0x00000031))
	{
		if (mbPressedTab == 0)
		{
			combatManager->mbAbilityMode = !combatManager->mbAbilityMode;
		}
		mbPressedTab = 1;
	}
	else
	...
	if (GameInputManager.IsTriggered(0x00000007)) //Spacebar
	{
		...
		{
			mbPressedSpace = 1;
			combatManager->delayedAbility = nullptr;
			if (avatar->mbSupported || CreatureGameData.mNumFlapsAllowed > 0)
			{
				...
				avatar->DoJump(50 * (!mbHasBeamedDown));
			}
			else
			{
	...
	if (leftClick) //Left click
	{
		...
		{
			combatManager->SelectCombatant(combatManager->mpHoveredCombatant);
		}
	}
}
